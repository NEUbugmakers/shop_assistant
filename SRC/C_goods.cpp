...
C_Goods C_GoodsCreat(char sort[],float out, char name[], char code[]) {//创建C_Goods,失败返回的商品编码为空
    C_Goods newGoods;
    qDebug()<<strlen(name);
    if (code==NULL||name==NULL||sort==NULL||strlen(sort) == 0 || out <= 0 || strlen(name) == 0 || strlen(code) == 0||strlen(sort)+strlen(code)>=C_CODE_LEN-1||strlen(name)>=C_NAME_LEN-1) {
       newGoods.code[0] = 0;
       return newGoods;
    }
...
C_goodsReturn* C_goodsSell(C_Goods* goods, char batch, int amount,int flag) {//商品出售
    Rank r = C_goodsGetBatch(goods, batch,1);
    if(r == -1)//未找到
        return NULL;
    C_goodsInfo* pos = (C_goodsInfo*)B_listGetRank(goods->C_shelfInfo, r);
    if(pos->C_amount<amount)//没有这么多商品
        return NULL;
    C_goodsReturn* price = (C_goodsReturn*)malloc(sizeof(C_goodsReturn));
    price->C_in = pos->C_in;
    price->C_out = goods->C_out;
    strcpy(price->name,goods->name);
    if(flag == 0){
        pos->C_amount -= amount;
...
C_Goods C_goodsRotPreWarning(C_Goods* goods,int pre) {//提前pre天进行过期报警,未过期则返回空信息，过期则返回仅包含过期商品的列表
    ...
    Date now = B_Time_I();
    while (travel != NULL) {
        if (B_DateIsSmall(((C_goodsInfo*)travel->_elem)->C_rotDate, B_DayLater(&now, pre)))
            C_goodsShelfAdd(&rot, (C_goodsInfo*)travel->_elem);
        travel = B_listNextNode(travel);
    }
...
C_Goods C_goodsGetRot(C_Goods* goods) {//获取过期商品
    return C_goodsRotPreWarning(goods, 7);
}
...
int C_goodsModify(C_Goods* goods, char name[], float price) {//修改商品信息
    if (strlen(name) == 0 || price <= 0||strlen(name)>=C_NAME_LEN-1)
        return 0;
    else {
...
void C_goodsaddsaleinfo(C_sales* goods,int amount){//更新商品销量
    C_salesNode* node = (C_salesNode*)B_vectorGet(goods->salesinfo,goods->salesinfo->_size-1);
    if(node==NULL||B_DayPassed(node->date,B_Time_I())!=-1){//第一个交易记录或当日第一件商品
        C_salesNode t;
        t.date=B_Time_I();
        t.amount=0;
        B_vectorPushBack(goods->salesinfo,&t);
    }
    node = (C_salesNode*)B_vectorGet(goods->salesinfo,goods->salesinfo->_size-1);
    node->amount += amount;
}
int C_goodsGetMultiSales(C_sales* goods, int num) {//返回近几日的销量
    if(num<=0)
        return 0;
    int total = 0;
    int t = goods->salesinfo->_size - num;
    if(t<0)
        t=0;
    for (int i = goods->salesinfo->_size - 1; i >= 0; i--) {
        C_salesNode* node = (C_salesNode*)B_vectorGet(goods->salesinfo, i);
        Date test = B_DayLater(&node->date,num-1);
        int judge = B_DateIsSmall(B_Time_I(),test);
        if(judge == 0){
            break;
        }
        total += node->amount;
