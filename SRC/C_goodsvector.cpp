...
C_Goods* C_goodsVectorFind_P(C_goodsVector* goodsVector, char code[]) {//根据编码查找商品，成功则返回商品指针，失败则返回NULL
    Rank r = C_goodsVectorFindInterval(goodsVector, code, 0, goodsVector->vector->_size);
    if(r<0)
        return NULL;
    C_Goods* goods = (C_Goods*)B_vectorGet(goodsVector->vector, r);
    if (strcmp(goods->code, code) == 0)
        return goods;
    else
        return NULL;
}
...
int C_goodsVectorInsert(C_goodsVector* goodsVector, C_Goods* e) {//插入,成功返回1，失败返回0
    ...
    if (r>=0&&strcmp((*(C_Goods*)B_vectorGet(goodsVector->vector, r)).code, e->code) == 0)//商品条目已经存在，插入失败
        return 0;
    else {
